...
try {
	if (isset($_SESSION[$session_logged])) {
		...
		$stato		= isset($request->stato)		? $request->stato		: null;

		if ($stato) {
			$params = [isset($stato->st_id) ? $stato->st_id : 0];
			$sql = "DELETE FROM stati WHERE st_id = ?";
			$query = query($beansMaps->StatoBean->dbh, $sql, $params);
			if ($query->status) {
			...
			$response->response = "stato is null";
		}
	} else {
